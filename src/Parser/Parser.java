...
public class Parser {
	...
	private void registraIdentificadorAtual() throws SemanticError {
		if (this.symbolTable.containsKey(this.currentScope() + "_" + this.token.getValue())) {
			throw new SemanticErrorIdAlready(this.token.getValue(), this.token.getLine(),
					this.lexicalAnalyzer.lineError());
		}
		this.token.setScope(this.currentScope());
		symbolTable.put(this.currentScope() + "_" + this.token.getValue(),
				new SemanticToken(this.token, this.currentType));
	}
		...
		while (!this.tokenEquals(Tag.EOF)) {
			...
			if (this.first.tipo_dado.contains(this.token.getMark())) {
				...
				if (this.token.getMark().getDescription().equals("ID")
						|| this.token.getMark().getDescription().equals("MAIN")) {
	...
	public void programa()
			throws OutOfRange, EmptyCharacter, LexicalError, ManyCharacters, SyntaxError, SemanticError, IOException {
		...
		if (!this.tokenEquals(Tag.EOF))
			throw new SyntaxError("Unexpected " + this.token.getValue(), this.token.getLine(),
					this.lexicalAnalyzer.lineError());
	}

	private void conteudo_antes_main()
	...
	private void passada_de_parametros()
			throws LexicalError, ManyCharacters, EmptyCharacter, SyntaxError, OutOfRange, SemanticError {
		String currentFunctionCallParams = "(";
		if (!this.first.passada_de_parametros.contains(token.getMark())) {
			if (this.tokenEquals(Tag.SP_CHAR_CLOSE_PARENTHESES)
					&& this.currentFunctionToken.getFunctionParams().size() > 0) {
				throw new SemanticError(
						"Semantic Error! Function '" + this.currentFunctionToken.getValue() + "' needs "
								+ this.currentFunctionToken.getFunctionParams().size() + " parameters, but found 0.",
						this.token.getLine(), this.lexicalAnalyzer.lineError());
			}
		...
		if (this.currentFunctionToken.getFunctionParams().size() == 0) {
			throw new SemanticError(
					"Semantic Error! Function '" + this.currentFunctionToken.getValue() + "' needs "
							+ this.currentFunctionToken.getFunctionParams().size()
							+ " parameters, but parameter(s) is(are) being passed.",
					this.token.getLine(), this.lexicalAnalyzer.lineError());
		}
		...
		if (identifier == null && this.tokenEquals(Tag.ID)) {
			throw new SemanticError("Identifier '" + this.token.getValue() + "' is not defined.", this.token.getLine(),
					this.lexicalAnalyzer.lineError());
		} else if (!this.token.getMark().getDescription()
				.equals(SemanticType.valueOf(currentParamType.name()).getDescription())) {
			throw new SemanticError(currentParamType.getDescription(),
					SemanticType.valueOf(this.token.getMark().name()).getDescription(), this.token.getLine(),
					this.lexicalAnalyzer.lineError());
		}
	...
	private String fim_passada_de_parametros(int i_passada_de_parametros, String currentFunctionCallParams)
		...
		if (this.first.fim_passada_de_parametros.contains(token.getMark())) {
			...
			if (i_passada_de_parametros + 1 > params_quantity)
				throw new SemanticError(
						"Semantic Error! Function '" + this.currentFunctionToken.getValue() + "' has only "
								+ params_quantity + " parameters.",
						this.token.getLine(), this.lexicalAnalyzer.lineError());
			...
			throw new SemanticError(
					"Semantic Error! Function '" + this.currentFunctionToken.getValue() + "' needs " + params_quantity
							+ " parameters, but found " + i_passada_de_parametros + ".",
					this.token.getLine(), this.lexicalAnalyzer.lineError());
		return currentFunctionCallParams;
	}

	private void atribuicao_ou_chamada()
			throws LexicalError, ManyCharacters, EmptyCharacter, SyntaxError, OutOfRange, SemanticError {
		...
		if (!this.currentFunctionReturnType.equals(SemanticType.CHAR)) {
			...
			if (this.token.getMark().equals(Tag.ID)) {
				...
				if (aux == null)
					...
					throw new SemanticError(
							"Identifier '" + this.token.getValue() + "'" + " is not of the function return type "
									+ SemanticType.CHAR.getDescription().toLowerCase(),
							this.token.getLine(), this.lexicalAnalyzer.lineError());
			} else if (!this.token.getMark().equals(Tag.CHARACTER))
				throw new SemanticError(
						"Identifier '" + this.token.getValue() + "'" + " is not of the function return type "
								+ Tag.CHARACTER.getDescription().toLowerCase(),
						this.token.getLine(), this.lexicalAnalyzer.lineError());
			this.nextToken();
		}
	...
	void comando() throws LexicalError, ManyCharacters, EmptyCharacter, SyntaxError, OutOfRange, SemanticError {
		if (this.tokenEquals(Tag.IF)) {
			...
			if (identifier == null && this.currentFunctionToken == null) {
				throw new SemanticErrorIdNotdefined(this.token.getValue(), this.token.getLine(),
						this.lexicalAnalyzer.lineError());
			}
	...
	private void fim_declaracao_for()
			throws SyntaxError, LexicalError, ManyCharacters, EmptyCharacter, OutOfRange, SemanticError {
		this.nextToken();
	...
	private void declaracao_for_inline()
			throws LexicalError, ManyCharacters, EmptyCharacter, SyntaxError, OutOfRange, SemanticError {
	...
	private void condicao(boolean invertReloperand)
		...
		if (this.token.getMark().name().equals("ID")) {
			...
			case "INT":
			case "INTEGER":
				this.currentType = SemanticType.INT;
				break;
			case "FLOAT":
			case "FLOATING_POINT":
				this.currentType = SemanticType.FLOAT;
				break;
			case "CHAR":
			case "CHARACTER":
				this.currentType = SemanticType.CHAR;
				break;
			}
		}
		this.expressaoAritmetica(false);
	...
	private Token invertReloperand(Token token) {
		...
		case "REL_OP_EQUALS":
			token = new Token(Tag.REL_OP_NOT_EQUAL_TO, "!=", token.getScope());
			break;
		case "REL_OP_NOT_EQUAL_TO":
			token = new Token(Tag.REL_OP_EQUALS, "==", token.getScope());
			break;
		case "REL_OP_GREATER_THAN":
			token = new Token(Tag.REL_OP_LESS_THAN, "<", token.getScope());
			break;
		case "REL_OP_LESS_THAN":
			token = new Token(Tag.REL_OP_GREATER_THAN, ">", token.getScope());
			break;
		case "REL_OP_GREATER_THAN_OR_EQUALS_TO":
			token = new Token(Tag.REL_OP_LESS_THAN_OR_EQUALS_TO, "<=", token.getScope());
			break;
		case "REL_OP_LESS_THAN_OR_EQUALS_TO":
			token = new Token(Tag.REL_OP_GREATER_THAN_OR_EQUALS_TO, ">=", token.getScope());
			break;
		}
		return token;
	}
	...
	private void valor_atribuicao()
			throws LexicalError, ManyCharacters, EmptyCharacter, SyntaxError, OutOfRange, SemanticError {
		this.nextToken();
		if (!this.first.operacao.contains(this.token.getMark()))
			throw new SyntaxError("Invalid attribuition declaration", this.token.getLine(),
					this.lexicalAnalyzer.lineError());
	...
	private void declaracaoDeFuncao()
			throws LexicalError, SyntaxError, ManyCharacters, EmptyCharacter, OutOfRange, SemanticError {
	...
	private void expressaoAritmetica(boolean atribuicao)
			throws SyntaxError, LexicalError, ManyCharacters, EmptyCharacter, OutOfRange, SemanticError {
		if (atribuicao)
	...
	private void expressaoAritmeticaLinha(boolean atribuicao)
			throws SyntaxError, LexicalError, ManyCharacters, EmptyCharacter, OutOfRange, SemanticError {
		if (!this.first.expressao_aritmetica_linha.contains(this.token.getMark())
				&& !this.first.operando.contains(this.token.getMark()))
			return;
		else if (this.first.operando.contains(this.token.getMark()))
			throw new SemanticError("arithmetic operator", this.token.getValue(), this.token.getLine(),
					this.lexicalAnalyzer.lineError());
	...
	private void termoLinha(boolean atribuicao)
			throws LexicalError, ManyCharacters, EmptyCharacter, OutOfRange, SyntaxError, SemanticError {
		...
		if (this.token.getMark().getDescription().equals("ID")) {
			...
			if (identifier == null) {
				...
				if (functionIdentifier == null)
					throw new SemanticError("Function '" + this.token.getValue() + "' is not defined",
							this.token.getLine(), this.lexicalAnalyzer.lineError());
					...
					throw new SemanticError(this.currentType.getDescription().toLowerCase(),
							functionIdentifier.getReturnType().getDescription().toLowerCase(), this.token.getLine(),
							this.lexicalAnalyzer.lineError());
			} else if (!this.currentType.getDescription().equals(identifier.getType().getDescription()))
				throw new SemanticError(this.currentType.getDescription(), identifier.getType().getDescription(),
						this.token.getLine(), this.lexicalAnalyzer.lineError());
	...
	private void gerarTokensExpressoesAritmeticas(int totalScopes, LinkedList<ExpressionToken> expressionTokens,
		...
		for (Token t : this.expressaoDaAtribuicaoAtual) {
			if (i == this.expressaoDaAtribuicaoAtual.size() - 1)
				break;
			if (t.getMark().equals(Tag.SP_CHAR_OPEN_PARENTHESES))
				forCurrentScope += 1;
	...
	private void gerarTokensTemporariosDeExpressoesAritmeticas(boolean declaracao) {
		...
		if (temporaryTokens.size() == 0)
			this.geraDefinicaoDeVariavel(declaracao, this.attribuitionCurrentToken.getValue(),
	...
	private void imprimirTokensTemporariosDeExpressoesAritmeticas(boolean declaracao,
			LinkedList<ExpressionToken> temporaryTokens) {
		LinkedList<Integer> idsTaken = new LinkedList<>();
		int i = 0;
		int aux = 0;
		...
		this.geraDefinicaoDeVariavel(declaracao, this.attribuitionCurrentToken.getValue(),
				temporaryTokens.get(temporaryTokens.size() - 1).getName());
		System.out.println();
	}
	...
	private boolean registradorTemporarioDeclarado(String name) {
		if (this.currentType.getDescription().equals("INTEGER")
				&& temporaryIntegers.containsKey(this.currentScope() + "_" + name))
			return true;
		else
			return this.currentType.getDescription().equals("FLOATING_POINT")
					&& temporaryFloats.containsKey(this.currentScope() + "_" + name);
	}
